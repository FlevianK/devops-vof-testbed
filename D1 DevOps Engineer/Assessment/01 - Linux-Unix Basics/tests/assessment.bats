...
@test "create a nested directory named 'foo/bar/baz'" {
  run_command mkdir -p foo/bar/baz
...
EOF
  ...
  run_command find . -type f -name "file-*"
  ...
  run_command find . -type d -name "file-*"
...
@test "format block device '/dev/loop0' as ext4 and mount to '/mnt/virtual'" {
  ...
  # run the command below to format device '/dev/loop0' as 'ext4'
  run_command sudo mkfs.ext4 /dev/loop0 

  # run the command below to mount '/dev/loop0' to '/mnt/virtual'
  run_command sudo mkdir -p /mnt/virtual
  run_command sudo mount /dev/loop0 /mnt/virtual
  ...
  mount_info=$(mount)
  echo -e "$mount_info" | grep "/dev/loop0" | grep "/mnt/virtual" | grep "ext4"
}
...
EOF
  ...
  run_command sh -c "echo 'SOME MESSAGE' > ${tmp_dir}/output-file"
  [ "$status" -eq 0 ]
