...
create_application_yml() {
	cat <<EOF > /home/vof/app/vof-tracker/config/application.yml
...
create_secrets_yml() {
  cat <<EOF > /home/vof/app/vof-tracker/config/secrets.yml
production:
...
start_app() {
  local app_root="/home/vof/app/vof-tracker"

  sudo -u vof bash -c "mkdir -p /home/vof/app/vof-tracker/log"
